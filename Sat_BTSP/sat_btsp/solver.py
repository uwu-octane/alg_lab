...
class BTSPSolverSat:
    ...
    def __init__(self, points: Iterable[util.Node], solver: str = "Gluecard4",
                 solution: List[util.Edge] = None):
    ...
    def __add_degree_constraints(self):
        """
        Add constraints that assure  |N(v)| == 2 for all v in V.
        """
        for v in self.graph.nodes:
            ...
            negative_edge_vars = [-v for v in edge_vars]
            self.solver.add_atmost(edge_vars, 2)
            self.solver.add_atmost(negative_edge_vars, len(edge_vars) - 2)
    ...
    def solve(self, solution_lb: List[Edge] = None, solution_ub: List[Edge] = None):
        """
        ...
        if self.best_solution is not None:
            ...
            print(f"Starting with lb_bottleneck:", {math.dist(*self.all_edges[lb])})
            print(f"Starting with bottleneck:", {math.dist(*self.all_edges[ub])})
        if solution_lb is not None and solution_ub is not None:
            lb = self.__max_index(solution_lb)
            ub = self.__max_index(solution_ub)
            print(f"Starting with ub_bottleneck:", {math.dist(*self.all_edges[ub])})
            print(f"Starting with lb_bottleneck:", {math.dist(*self.all_edges[lb])})
        while lb < ub - 1:
            ...
            if actual_index:
                ...
                lb = mid
        print(f"Found bottleneck: {math.dist(*self.all_edges[ub])}!")
