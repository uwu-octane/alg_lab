...
class DBSTSolverCP:
    ...
    def __make_vars(self):
        ...
        """
        在 DCBST 算法中，为每个节点都创建了一个深度变量，用于表示该节点在生成树中的深度。
        为了限制生成树的度数，需要对每个节点的深度变量设置一个上限值，使得该节点在生成树中的深度不超过上限值。
        """
        self.depth_vars = {v: self.model.NewIntVar(0, self.n - 1, f'd_{v}') for v in range(self.n)}

        """
        这段代码是在创建一个整数变量 `bottleneck_var`，用于表示生成树中的最大边权。
        具体来说，使用 Google OR-Tools 提供的 `NewIntVar()` 方法，
        创建了一个下界为 0、上界为 `max_distance` 的整数变量，并将其命名为 `b`。
        在 DCBST 算法中，需要将生成树中的最大边权最小化。
        因此，在创建变量后，还需要将其设为目标函数的最小化项，即调用 `model.Minimize()` 方法，
        并将 `bottleneck_var` 作为参数传入。
        这样，Google OR-Tools 便会在求解过程中自动调整 `bottleneck_var` 的取值，使得生成树中的最大边权尽可能小。
        """
    ...
    def __add_depth_constraints(self):
        ...
        for (v, w), x_vw in self.edge_vars.items():
            """
            `OnlyEnforceIf` 是 Google OR-Tools 提供的一个约束条件，它表示只有当 `x_vw` 为真（即边 $(v,w)$ 被选择）时，约束条件才被应用，
            否则约束条件不会被应用。在这个例子中，当且仅当边 $(v,w)$ 被选择时，约束条件 $d(w) = d(v) + 1$ 才会被应用。
            这个约束条件保证了生成树的深度限制。
            """
